...
static int32_t ILPS22QS_0_Probe(uint32_t Functions)
{
  ...
  io_ctx.Init        = CUSTOM_ILPS22QS_0_I2C_Init;
  io_ctx.DeInit      = CUSTOM_ILPS22QS_0_I2C_DeInit;
  io_ctx.ReadReg     = CUSTOM_ILPS22QS_0_I2C_ReadReg;
  io_ctx.WriteReg    = CUSTOM_ILPS22QS_0_I2C_WriteReg;
  ...
  else
  {
    ...
    EnvCtx[CUSTOM_ILPS22QS_0].Functions = ((uint32_t)cap.Temperature)
                                        | ((uint32_t)cap.Pressure << 1)
                                        | ((uint32_t)cap.Humidity << 2)
                                        | ((uint32_t)cap.Gas      << 3);

    EnvCompObj[CUSTOM_ILPS22QS_0] = &ilps22qs_obj_0;
    /* The second cast (void *) is added to bypass Misra R11.3 rule */
    EnvDrv[CUSTOM_ILPS22QS_0] = (ENV_SENSOR_CommonDrv_t *)(void *)&ILPS22QS_COMMON_Driver;

    if ((ret == BSP_ERROR_NONE) && ((Functions & ENV_TEMPERATURE) == ENV_TEMPERATURE) && (cap.Temperature == 1U))
    {
      /* The second cast (void *) is added to bypass Misra R11.3 rule */
      EnvFuncDrv[CUSTOM_ILPS22QS_0][FunctionIndex[ENV_TEMPERATURE]] = (ENV_SENSOR_FuncDrv_t *)(void *)&ILPS22QS_TEMP_Driver;

      if (EnvDrv[CUSTOM_ILPS22QS_0]->Init(EnvCompObj[CUSTOM_ILPS22QS_0]) != ILPS22QS_OK)
      {
        ret = BSP_ERROR_COMPONENT_FAILURE;
      }
      else
    ...
    {
      /* The second cast (void *) is added to bypass Misra R11.3 rule */
      EnvFuncDrv[CUSTOM_ILPS22QS_0][FunctionIndex[ENV_PRESSURE]] = (ENV_SENSOR_FuncDrv_t *)(void *)&ILPS22QS_PRESS_Driver;
